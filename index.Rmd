...
cdata <- summaryBy(Freq ~ Class + Survived, data=titanic.df, FUN=c(sum))
colnames(cdata) <- c("Passenger Class", "Survived", "Count")
print (cdata)
...
```{r echo=FALSE}
cdata <- summaryBy(Freq ~ Sex + Survived, data=titanic.df, FUN=c(sum))
colnames(cdata) <- c("Gender", "Survived", "Count")
print (cdata)
...
```{r echo=FALSE}
cdata <- summaryBy(Freq ~ Age + Survived, data=titanic.df, FUN=c(sum))
colnames(cdata) <- c("Age", "Survived", "Count")
print (cdata)
